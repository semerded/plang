...
class Logger:
    ...
    @staticmethod
    def log(level: Callable, message: str) -> None:
    ...
    def _check_log_file_size() -> bool:
        num_files = len([f for f in os.listdir(data.log_file_location) if os.path.isfile(os.path.join(data.log_file_location, f))])
    ...
    def _cleanup_oldest_log_file():
        ...
        os.remove(os.path.join(data.log_file_location, f'log-runtime={min(dates).strftime("%Y-%m-%d_%H-%M-%S.log")}'))
...
class Messenger:
    ...
    def success(message: str) -> None:
        ...
        Logger.log(logging.INFO, "SUCCES: " + message)
    ...
    def error(error: str | Exception, terminate: bool = False) -> NoReturn | None:
        if terminate and not data.debugging:
            ...
            Logger.log(logging.CRITICAL, str(error))
